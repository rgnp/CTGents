...
## 最新更新（2026-05-29 v0.5）

### 1.5 项目规范检查 ✅

- [x] `check_project` 工具：六维度扫描项目是否符合 AI Agent 开发规范
  （命令/测试/结构/代码风格/Git/边界），0-100 打分 + 四级评级
- [x] `generate_agents_md` 工具：自动扫描项目并生成 AGENTS.md
- [x] `AGENTS.md`：面向 AI 智能体的项目操作手册（命令/测试/结构/风格/Git/边界）
- [x] 三级边界系统：✅始终执行 / ⚠️事先询问 / 🚫绝不执行
- [x] 自动修复：`check_project(fix=True)` 可自动生成缺失的 AGENTS.md

**涉及文件：** `src/tools/lint.py`（新建）、`AGENTS.md`（新建）
