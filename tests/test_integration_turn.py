...
DRIFT 注意：`_drive_turn` 镜像 main() 的每轮管线（main.py:413-437）。main 那段
改了顺序，这里不会自动跟——改 main 管线时同步本文件。让网"权威化"的正解是把每轮
管线抽成 main.process_turn() 再让本网调它，本期先不抽（避免扩面、风险）。
"""
...
def _drive_turn(ctx: CacheContext, user_input: str) -> None:
    """镜像 main() 每轮管线（main.py:413-437）——见文件头 DRIFT 注意。"""
    main._inject_memory_signal(ctx, user_input)
    pre_msgs = main._preread_files(user_input, ctx)
    if pre_msgs:
        contents = "\n\n".join(m["content"] for m in pre_msgs)
        user_input = (f"[以下文件已预读，可直接基于其内容回答]\n\n{contents}\n\n"
                      f"── 用户问题 ──\n{user_input}")
    llm.run_conversation(ctx, user_input, on_token=lambda _t: None,
                         on_tool=lambda *_a: None, session_id="")
    main._inject_completion_audit(ctx)
    main._inject_citation_audit(ctx)
