...
_HARVEST_SYSTEM = (
    ...
    "工作习惯 / 长期方向与目标。其中『长期方向与目标』务必单列『## 长期方向 / 目标』一面，"
    "抓住他长期所在的领域、攻关方向、目标场景或目标会议·期刊、长期目标（这是『他是谁、"
    "长期在做什么』）——别把它抽象成泛泛的一句。不要写泛泛而谈、临时性、或纯推测的内容；"
    "拿不准就不写。\n"
    "2) 区分长期与易变：档案只放长期稳定的身份与目标（领域、目标会议、长期方向）；"
    "当前还在变动的具体课题 / 候选方案 / 选题中的子方向【不要】写进来——它们会过时，"
    "留给项目知识库，别污染这份每轮都注入的长期档案。\n"
