...
### 执行工具
可用工具已在系统中注册，直接用 function calling 调用即可。同一回复可调多个工具，系统会依次执行并汇总结果。
...
class GoalRunner:
    ...
    def run(self) -> bool:
        ...
        for it in range(1, MAX_ITERATIONS + 1):
            ...
            # ── 解析 LLM 输出 ──
            # function calling → 可能多个 tool_call；文本 JSON → 单个 action
            actions = self._parse_actions(tool_calls, content)
            if not actions:
                self._handle_error(it, f"无法解析输出: {str(content)[:200]}")
                continue

            first = actions[0]
            action_type = first.get("action", "")
            reasoning = first.get("reasoning", "")

            # ── 更新计划/当前步骤 ──
            if first.get("plan"):
                self.state.plan = first["plan"]
            if first.get("current_step"):
                self.state.current_step = first["current_step"]

            # ── 完成 / 失败 ──
            if action_type == "done":
                summary = first.get("summary", "完成")
                self._say(f"✅ {summary}\n")
                self.state.finished = True
                break

            if action_type == "fail":
                reason = first.get("reason", "未知原因")
                self._say(f"❌ {reason}\n")
            ...
            for idx, act in enumerate(actions):
                ...
                if idx == 0:
                    ...
                    self._say(f"  ├─ {tool_name} — {act.get('reasoning', '')[:60]}\n")

                result, error = self._exec(tool_name, tool_args)
                tool_results.append((tool_name, tool_args, result, error))
            ...
            # ── 合并记录历史（多个工具算一步，节省窗口） ──
            if tool_results:
                names = "; ".join(t[0] for t in tool_results)
                combined_result = "\n".join(
                    f"[{t[0]}] {t[2][:300]}" for t in tool_results if t[2]
                )[:2000]
                combined_error = "\n".join(
                    f"[{t[0]}] {t[3]}" for t in tool_results if t[3]
                )[:500]
                self.state.add_history(
                    it, names, tool_results[0][0],
                    combined_result, combined_error,
                )

            if first.get("mark_done"):
                self.state.mark_done(first["mark_done"])
        # ── 结束 ──
    ...
    def _parse_actions(self, tool_calls: list[dict] | None,
                       content: str | None) -> list[dict] | None:
        """将 LLM 输出解析为 action 列表（支持多工具）。

        function calling 时每个 tool_call 转为一个 action；
        文本 JSON 时转为单个 action 列表。
        """
        # ── 情况1: function calling → 多工具 ──
        if tool_calls:
            meta: dict = {}
            if content:
            ...
            for tc in tool_calls:
                ...
                try:
                    ...
                    args = {}
                actions.append({"action": "tool_call", "tool": name, "args": args})

            if actions:
                actions[0]["reasoning"] = meta.get("reasoning", content or "")
                actions[0]["plan"] = meta.get("plan", [])
                actions[0]["current_step"] = meta.get("current_step", "")
                actions[0]["mark_done"] = meta.get("mark_done", "")
            return actions

        # ── 情况2: 文本 JSON → 单 action ──
        single = _parse_action(content)
        if single:
            return [single]
