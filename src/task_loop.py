...
def _drive_prompt() -> str:
    """自主续跑每步的驱动输入：以"agent 对自己说话"的语气，不是"用户对我说话"。

    只给自主执行的姿态 + 显式停止信号的出口（task_done/need_user 走工具，被循环认账，
    取代"在回复里说停"——后者会被自动续跑覆盖）。
    """
    return (
        "[自主推进] 我按 current.md 做下一步，做完更新步骤标记"
        "（进度记在步骤后，如 47/250）。\n"
        "停止信号（只在回复里说会被续跑覆盖，必须走工具）：\n"
        "· 全部完成 → 全标 [x] 并调 task_done\n"
        "· 需要对方拍板 → 调 need_user 说明情况（或标 [!]）\n"
        "· 计划不再匹配 → 用 update_plan 调整（增删改/重排序）\n"
        "常规步骤直接做，不用停。"
    )


def _review_prompt() -> str:
    """计划审查的驱动输入：agent 对自己说话的语气。
