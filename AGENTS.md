...
9. **断言缺陷前先验证** — 声称代码有 bug、缺功能、或需要修改前，必须用工具确认。说"命中率未展示"之前先 `grep_code "hit_ratio" commands.py`；说"缺熔断器"之前先 `grep_code "recent_summary\|_compacted" llm.py`。不确定时默认「它可能已经解决了」。本对话中 5 条缺陷分析 3 条是幻觉、功能清单 #1 #2 已实现但被标记为未做——根因全是跳过验证。

### 任务分解

10. **复杂任务先计划** — 涉及 3+ 文件或架构决策时，先用 `think` 工具规划，再执行。
