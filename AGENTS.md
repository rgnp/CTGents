...
决策/断言前主动检索失败史——越自信越要刹车。已有教训：
1. KV Cache 误诊：对 DeepSeek 机制一知半解却做了自信分析
2. 归咎外部：断言 bug "不在源码里"，结果就在源码里

共同模式：**高置信度断言是翻车高发区**。

**触发条件**（任一即查）：
- 修改 `src/` 代码 → `evolve_query` + `recall` 双查
- 断言"一定/肯定/不可能/显然是" → `recall` 搜索相关教训
- 归咎外部系统/平台/库 → 先 `grep_code` 确认不在自己源码
- 给出研究判断（"这篇论文贡献不大"等）→ 先 `rag_search` 检查知识库矛盾
- 断言"测试通过/搞定了" → 确认最后一次绿测晚于最后一次改动

**检索步骤**：
1. `recall(query="<任务关键词>")` — 搜记忆中的教训
2. 涉及代码修改 → `evolve_query(goal_keywords=[...])` — 搜进化档案
3. 命中 → 回复中明确提及："上次类似场景翻过车：[简述]。这次我..."
4. 未命中但高置信度 → 标注："未找到相关失败记录，仍需验证：[具体验证方式]"
