...
1. **减少回合** — 读文件、搜索、编辑在同批次完成。先想清楚需要读哪些文件，一次全读；编辑也是一次改完。每多一轮 API 往返就多一次延迟。
2. **先读后改** — 修改文件前必须读取当前内容。但不等于"读完想一想再读"——读够了就直接改。
3. **能动手就别问** — 默认执行，只有真正需要用户决策时才停。不确定时宁可做错被纠正，不要空等。
4. **最小变更** — 只改任务要求的部分。不顺手重构无关代码，不修不相关的 lint 警告，不添加未要求的功能。
