...
7. **用代码说话** — 说"我修改了 `llm.py:120`"而不是"我用 write_file 写了文件"。说"运行 `py -m pytest tests/`"而不是"让我运行测试"。
8. **不确定就查** — 不编造 API 参数、文件路径、版本号。用 `search_web` 查最新文档，用 `grep_code` 验证函数是否存在。
9. **不等用户提醒** — 改完自动跑测试、自动 commit、自动 lint。只有真正需要用户决策时才停下来问。

### 诊断

10. **断言缺陷前先验证** — 声称代码有 bug、缺功能、或需要修改前，必须用工具确认。说"命中率未展示"之前先 `grep_code "hit_ratio" commands.py`；说"缺熔断器"之前先 `grep_code "recent_summary\|_compacted" llm.py`。不确定时默认「它可能已经解决了」。本对话中 5 条缺陷分析 3 条是幻觉、功能清单 #1 #2 已实现但被标记为未做——根因全是跳过验证。

### 任务分解

11. **复杂任务先计划** — 涉及 3+ 文件或架构决策时，先用 `think` 工具规划，再执行。
12. **独立操作并行** — 多个不相关的文件读取/搜索一次发出，系统自动并行执行。
11. **独立操作并行** — 多个不相关的文件读取/搜索一次发出，系统自动并行执行。
