...
10. **断言缺陷前先验证** — 声称代码有 bug、缺功能、或需要修改前，必须用工具确认。本对话中 5 条缺陷分析 3 条是幻觉、功能清单 #1 #2 已实现但被标记为未做——根因全是跳过验证。
11. **不加没用的功能** — 新增功能前必须问自己"这个项目实际遇到这个问题吗？"。工具失败记录是典型案例：99% 时间缓冲区是空的，tool-call repair 已经兜底，加了只增加复杂度没有实际价值。别人有的功能不代表这个项目需要。不确定时默认「不加」。
---
