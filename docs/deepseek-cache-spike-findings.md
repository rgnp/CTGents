...
### 3.4 缺口随 payload 体积增大（量级，非精确数）

`#16` 在 ~33k token 量级缺口约 12k；`#37` 在 ~82k 量级缺口约 50k（均为估算，见 §六）。
→ 对话越大，被一次性淘汰的前缀越多。
...
## 六、检测口径：结构判断可信，量级估算不精确

取证刻意把两件事分开，避免把估算当事实：

**(1) 结构判断 `client_prefix_status`——只看字节/哈希，可证伪、可信。** `payload_diff` 先对每对
相邻请求纯结构地分类（不掺任何 token 数）：

| status | 判据 | 责任 |
|---|---|---|
| `tools_changed` | tools_hash 变 | 客户端（前缀作废） |
| `history_mutated` | first_diff 落在对话中段 | 客户端 bug |
| `tail_only_changed` | 对话逐字节同、仅尾部牙浮动 | 设计成本 |
| `pure_append` | 上轮整段是本轮逐字节前缀 | 只追加，未动旧内容 |

**「服务端缓存未命中」只在 `pure_append` 下判定**——此时任何超出「追加尾」的 miss 都没有客户端解释。
其余三种 status 下命中下降都有客户端侧原因，不归服务端。

**(2) 缺口 token 数——只是估算，不是精确事实。** 判词写成
「按上轮prompt估算可复用≈X，实命中Y，缺口≈Z（估算）」，**不写「服务端吃掉=Z」这种硬数字**。
原因：基准「上轮 prompt_tokens」本身会随 cache hit/miss 漂移（见第四节），缺口因而带 ±几 k 误差。

**一句话**：判定「服务端未命中」可信（方向铁定）；缺口 token 数不精确（只在「几万量级、随 payload
增大」的粒度上使用，不应钉到具体数）。
